...
import { BookOpen, Users, GraduationCap, Filter, X, ChevronDown } from "lucide-react"
...
export default function StoriesPage() {
  ...
  const hasActiveFilters = keywordFilter || universityFilter || facultyFilter || yearFilter || campusFilter
  const activeFilterCount = [keywordFilter, universityFilter, facultyFilter, yearFilter, campusFilter].filter(Boolean).length
        ...
        <div className="lg:hidden mb-4 space-y-3">
          ...
          <div className="bg-white rounded-xl shadow-md p-4 border" style={{ borderColor: '#bac9d0' }}>
            <form onSubmit={handleKeywordSearch} className="flex gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-2 text-xs font-semibold text-white rounded-lg shadow-md"
                style={{ background: 'linear-gradient(to right, #044465, #055a7a)' }}
              >
                検索
              </button>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold" style={{ color: '#044465' }}>絞り込み検索</span>
              {activeFilterCount > 0 && (
                <span className="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white rounded-full" style={{ backgroundColor: '#044465' }}>
                  {activeFilterCount}
                </span>
              )}
            </div>
          ...
          <>
            ...
            <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-xl lg:hidden max-h-[85vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: '#bac9d0' }}>
                <div className="flex items-center gap-2">
                  <Filter className="w-5 h-5" style={{ color: '#044465' }} />
                  <span className="text-lg font-bold" style={{ color: '#044465' }}>絞り込み検索</span>
                </div>
                <button
                  onClick={() => setIsFilterOpen(false)}
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-gray-600 text-lg">
                  {hasActiveFilters ? "条件に一致する体験記が見つかりませんでした" : "まだ体験記が投稿されていません"}
                </p>
                <p className="text-gray-500 text-sm mt-2">
                  {hasActiveFilters ? "フィルターを変更してみてください" : "最初の体験記を投稿してみませんか？"}
                </p>
              </div>
            ) : (
              <>
                        ...
                        <div className="mb-3 p-3 rounded-lg" style={{ backgroundColor: '#f0f4f5' }}>
                          {story.highSchoolName && (
                            <div className="flex items-center gap-1.5 mb-1.5">
                              ...
                              <p className="text-xs font-semibold leading-tight truncate" style={{ color: '#044465' }}>
                                {story.highSchoolName}
                              </p>
                            </div>
                          )}
